...
public class ArticleDaoImpl_Hibernate implements ArticleDao{
	...
	private int recordsPerPage = 4; // 預設值：每頁三筆
...
//		int startRecordNo = (pageNo - 1) * recordsPerPage;
		list = session.createQuery(hql).setParameter("ossanId", ossanId).list();
	...
	public ArticleBean queryArticle(int aId) {
		...
		article.setsContent(SystemUtils2018.clobToStringWindows(article.getContent()));
		return article;
	}
	@Override
